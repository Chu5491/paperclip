...
function accumulateUsage(
    ...
    const source =
        Object.keys(usageMetadata).length > 0 ? usageMetadata : usage;
...
export function parseGeminiJsonl(stdout: string) {
    ...
    for (const rawLine of stdout.split(/\r?\n/)) {
        ...
        if (
            type === "assistant" ||
            (type === "message" && asString(event.role, "") === "assistant")
        ) {
        ...
        if (type === "result") {
            ...
            accumulateUsage(
                usage,
                event.usage ?? event.usageMetadata ?? event.stats,
            );
            const resultText =
...
export function detectGeminiQuotaExhausted(input: {
    ...
    const exhausted = messages.some((line) =>
        GEMINI_QUOTA_EXHAUSTED_RE.test(line),
    );
    return {exhausted};
}
