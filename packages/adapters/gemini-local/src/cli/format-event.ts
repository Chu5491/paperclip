...
export function printGeminiStreamEvent(raw: string, _debug: boolean): void {
    ...
    if (type === "system") {
        ...
        if (subtype === "init") {
            ...
            const details = [
                ...
                .join(", ");
            console.log(
                pc.blue(`Gemini init${details ? ` (${details})` : ""}`),
            );
            return;
        }
        if (subtype === "error") {
    ...
    if (type === "tool_call") {
        ...
        console.log(
            pc.yellow(
                `tool_call: ${toolName}${subtype ? ` (${subtype})` : ""}`,
            ),
        );
        return;
    }
    ...
    if (type === "tool_use") {
        const name = asString(
            parsed.tool_name,
            asString(parsed.name, "tool"),
        );
        console.log(pc.yellow(`tool_call: ${name}`));
        const input = parsed.parameters ?? parsed.input ?? parsed.args;
        if (input !== undefined)
            console.log(pc.gray(stringifyUnknown(input)));
        return;
    }
    ...
    if (type === "tool_result") {
        ...
        const contentText =
            ...
            stringifyUnknown(
                parsed.output ?? parsed.result ?? parsed.response,
            );
        console.log(
            (isError ? pc.red : pc.cyan)(
                `tool_result${isError ? " (error)" : ""}`,
        ...
        if (subtype || isError) {
            console.log(
                (isError ? pc.red : pc.blue)(
                    `result: subtype=${subtype} is_error=${isError ? "true" : "false"}`,
                ),
            );
        }
        return;
